...
class RecurrencePlot:
    def __init__(
        self, signal: np.array, metric: Union[Callable[[np.array, np.array], np.ndarray], str], embedding: Embedding
    ):
    ...
    def generate(self, normalize: bool = True) -> np.ndarray:
        ...
        self._recurrence_plot = cdist(reshaped_signal, reshaped_signal, metric=self.metric)
...
if __name__ == "__main__":
    ...
    print(f"Dim: {dim} | Lag: {lag}")
    rp = RecurrencePlot(signal=sinusoid_signal, embedding=embedding, metric="euclidean")
